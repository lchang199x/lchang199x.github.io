...
3. 三种短期/支持分支：feature & hotfix & release
	...
	3. 发布分支处理一个新产品的发布，develop分支凑够一次发布的功能就会创建一个发布分支，主要进行文档的准备以及测试和bug修复，准备好以后要合并到master分支和develop分支，master分支合并发布分支之后应该打一个tag修改major或minor
